...
namespace SubSonic
{
    ...
    public class Constraint
    {
        ...
        public TableSchema.TableColumn Column { get; set; }
        ...
        public Select InSelect { get; set; }
        ...
        {
            Comparison = Comparison.Like;
            ParameterValue = String.Format("{0}{1}", val, query.Provider.GetWildCard());
            DbType = query.GetConstraintDbType(TableName, ColumnName, val);
            query.Constraints.Add(this);
        ...
        {
            Comparison = Comparison.Like;
            ParameterValue = String.Format("{0}{1}", query.Provider.GetWildCard(), val);
            DbType = query.GetConstraintDbType(TableName, ColumnName, val);
            query.Constraints.Add(this);
        ...
        /// <param name="val">The val.</param>
        /// <returns></returns>
        public SqlQuery ContainsString(string val)
        {
            Comparison = Comparison.Like;
            ParameterValue = String.Format("{0}{1}{2}", query.Provider.GetWildCard(), val, query.Provider.GetWildCard());
            DbType = query.GetConstraintDbType(TableName, ColumnName, val);
            query.Constraints.Add(this);
        ...
        public SqlQuery In(Select selectQuery)
        {
            ...
            if(selectQuery.SelectColumnList.Length + selectQuery.Aggregates.Count != 1)
                throw new SqlQueryException("You must specify exactly one column to return for the IN to be valid. Use Select(\"column\") to do this");
        ...
        public SqlQuery NotIn(Select selectQuery)
        {
			//validate that there is only one column in the columnlist
			if (selectQuery.SelectColumnList.Length + selectQuery.Aggregates.Count != 1)
                throw new SqlQueryException("You must specify exactly one column to return for the NOT IN to be valid. Use Select(\"column\") to do this");
